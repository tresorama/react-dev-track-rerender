...
type CtxState = { state: TodoState, setState: React.Dispatch<React.SetStateAction<TodoState>>; };
type CtxActions = TodoActions;

const ctxState = createContext({} as CtxState);
const ctxActions = createContext({} as CtxActions);


const useTodosContextState = () => {
  const value = useContext(ctxState);
  if (!value) {
    throw new Error('useTodosContextState must be used within a TodosProviderState');
  }
  return value;
};
const useTodosContextActions = () => {
  const value = useContext(ctxActions);
  if (!value) {
    throw new Error('useTodosContextActions must be used within a TodosProviderActions');
  }
  return value;
};

const TodosProviderState = ({ children }: { children: React.ReactNode; }) => {
  const [state, setState] = useState<TodoState>(() => ({
    raw: initialStateRaw,
    derived: deriveTodoState(initialState, initialStateRaw),
  }));
  const value: CtxState = { state, setState };
...
const TodosProviderActions = ({ children }: { children: React.ReactNode; }) => {
  const { setState } = useTodosContextState();
...
const useTodosState = () => useTodosContextState().state;
const useTodosActions = () => useTodosContextActions();
